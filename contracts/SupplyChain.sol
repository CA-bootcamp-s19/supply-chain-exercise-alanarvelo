...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require (msg.sender == _address, "Sender is not contract owner");
    _;
  }

  modifier isSeller (uint _sku) {
    require (msg.sender == items[_sku].seller, "Sender is not contract owner");
    _;
  }

  modifier isBuyer (uint _sku) {
    require (msg.sender == items[_sku].buyer, "Sender is not contract owner");
    _;
  }

  modifier paidEnough(uint _sku) {
    require(msg.value >= items[_sku].price, "Value sent does not meet item price");
    _;
  }
